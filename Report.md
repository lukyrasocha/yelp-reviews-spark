...
+-------------------+----------+------------+------------------+--------------+--------------------------+
```
Looking at the ratio of the usage of authentic language, we can see that on average people use authentic language with non-european cuisines more. That makes sense, since I believe that western society is more prone to look for `authenticity` in places that are foreign to them, which can also be very problematic. Furthermore looking into the ratio of how many of those authentic reviews were also negative we can't see any clear distinction between the two categories, but looking more specifically for `Chinese, Mexican, Italian and French` (the article states that restaurants most impacted by this difference serve Mexican and Chinese food), we can see that in a Chinese cuisine the use of negative authentic language is `13.72 %`, in Mexican `10.52`, in French only `8.61` and Italian `9.31` which might indicate a trend but further steps (going through the individual reviews for example) would need to be taken.

As the last test I did was to see whether there is some correlation between the usage of authentic language and star reviews. The study found that there’s a negative correlation (-0.17) between star rating and the amount reviewers use the word “authenticity” in Chinese and Mexican food. I looked into the correlation for non-european cuisines but didn't find any significant results (`correlation = 0.105`). 

### Limitations
As I mentioned before, the method I used is quite simplistic and has flaws. For example lets consider a review: `This austrian restaurant was soo authentic, really made me feel like I was in the middle of Vienna. Plus their coffee was very cheap compared to the usual prices in the city`. We can see that this review is positive, but since it contains the word `authentic` and also `cheap` we count it as a negative example of usage of `authenticity language`. Whereas review such as `This mexican restuarant with its cheap plastic tables reminded me of times when I visited Mexico for holidays`. We can see that even though, these two examples are basically in the same category one reflects the negative biased stereotypes that people have, whereas the other doesn't. The method also wouldn't catch a review for a French restaurant such as: `Old elegance at its best! Yes, the ambiance is lovely with all the fresh flowers` which also talks about authenticity but without directly stating the word `authentic`. (and we can imagine, that there are thousands of examples where people talk about south-american or south-asian cuisines using negative authentic language but not stating the words directly).

### Conclusion
The results I obtained show that non-european cuisines are in a lower-price categories than european restaurants, which proves some of the statements in the article. The method I used is however too simplistic and can generate many false positives and false negatives (in moments where the usage of authentic language is still harmful, but the review doesn't contain any of the negative words). Moreover testing correlation on the amount of authentic reviews with the average star reviews for the different cuisines didn't give any significant results. I wasn't unfortunately able to achieve such distinct and clear results as the researcher in the article, but I can assume that it is mainly because the researcher read through the reviews manually, which gave him a more nuanced view into the problem.
